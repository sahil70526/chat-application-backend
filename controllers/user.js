...
export const addProfileImage = async (req, res) => {
  try {
    if (req.file.path) res.status(200).json(req.file);
  } catch (error) {
    res.status(500).json({ error: error });
  }
}
...
export const getUserProfilePic = async (req, res) => {
  try {
    ...
    let filePath = path.join(__dirname, `/uploads/profileImage/${filename}`)
    let imageType = '';
    for (let i = filePath.length; i >= 0; i--) {
      if (filePath[i - 1] !== '.') imageType = filePath[i - 1] + imageType
    }
